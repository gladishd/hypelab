...
for (i in 1:length(filterByValues)) {
  ...
  highlights_modified <- highlights %>% 
    ...
    filter(vod_id == filterByValues[i]) # <--------------------------------change this to be for different video on demand IDs.
  corresponding_file <- eval(parse(text = paste('X', filterByValues[i], sep=''))) # <--------------------------------change this to be for different video on demand IDs.  
  ...
  for (i in 1:nrow(corresponding_file)) { # if the content offset is within the bounds
    tryCatch({
      ...
      # if (isHighlight) {
        bucket1 <- c(bucket1, paste(textBody, filterByValues[i], sep=""))
      } else {
        bucket2 <- c(bucket2, paste(textBody, filterByValues[i], sep=""))
      }
